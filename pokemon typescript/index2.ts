...
let showMorebtn = document.querySelector(".showmore") as HTMLButtonElement
let inputArea = document.querySelector("input")  as HTMLInputElement
let previewMain = document.querySelector(".preview") as HTMLElement
let SelectElem = document.querySelector("select") as HTMLSelectElement;
let limit = 20;
let offset = 0;
let mainPokemonData: PokemonDetails[];
let filterPokemonData:PokemonDetails[];
let oldData: PokemonDetails[];
...
async function fetchData(): Promise<PokemonResponse> {
...
async function getPokemonData() {
    ...
    let PokemonsMainData: Promise<PokemonDetails>[] = pokemons.results.map(async (item) => {
        let res: Response = await fetch(item.url);
        let data: PokemonDetails = await res.json();
        return data;
    });
...
async function generatePreview() {
    ...
    filterPokemonData.forEach((item:PokemonDetails) => {
